...
function SessionDetail({ enriched }: { enriched: EnrichedUser[] }) {
  ...
  const [messagesUser, setMessagesUser] = useState<EnrichedUser | null>(null);
    ...
    <div className="px-4 pb-4 space-y-3">
      ...
      <div className="space-y-1">
        {filtered.length === 0 ? (
          ...
          filtered.map((user) => (
            <LeadRow key={user.tiktokUserId} user={user} onShowMessages={setMessagesUser} />
          ))
        )}
      </div>
      ...
      {messagesUser && (
        <UserMessagesDialog
          nickname={messagesUser.nickname || messagesUser.displayId || "Anónimo"}
          commentTexts={messagesUser.commentTexts}
          onClose={() => setMessagesUser(null)}
        />
      )}
    </div>
  );
...
function LeadRow({ user, onShowMessages }: { user: EnrichedUser; onShowMessages?: (user: EnrichedUser) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 shrink-0">
        <button
          type="button"
          onClick={() => onShowMessages?.(user)}
          title="Ver todos los mensajes de este usuario"
          className="h-8 px-2.5 flex items-center gap-1 rounded-lg transition-all duration-200 text-xs font-medium"
          style={{
        ...
        >
          ...
          {user.commentTexts.length} mensajes
        </button>
        <button
          type="button"
          onClick={openDm}
