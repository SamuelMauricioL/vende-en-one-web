...
const OFFENSIVE_PATTERNS: RegExp[] = [
  ...
  // Más variantes de acoso a la vendedora
  /qu[eé]( (bien|tal|gran) )?mujer(ona|aza|on)?/i,
  /qu[eé] cuerpazo/i,
  /qu[eé] (guapa|linda|preciosa|belleza)/i,
  /^((oye|hey|eh) )?(guapa|linda|preciosa)/i,
  /^belleza/i,
  /^mamacita/i,
  /mami(ta)? (linda|hermosa|rica|buena)/i,
  /flaca (hermosa|linda|bonita|rica)/i,
  /(quieres|quisieras) ser mi (novia|enamorada)/i,
  /(me gustas|me encantas|te amo|me enamor[eé])/i,
  /te invito (a salir|un trago|una salida|a cenar)/i,
  /(sales|salgas) conmigo/i,
  /(c[uú]al es tu|cual es tu) (facebook|snapchat|telegram)/i,
  /tienes (onlyfans|snapchat|telegram)/i,
  /te compro a ti/i,
  /(yo |a ti )?te compro( a ti)?$/i,
  /cu[aá]nto (cobras|cuesta una noche)/i,
  /qui[eé]n es (la chica|la muchacha|ella|la de (atr[áa]s|la izquierda|la derecha|la par))/i,
  /c[oó]mo se llama (la|ella)/i,
  /pres[eé]ntame a tu (amiga|compañera|hermana)/i,
  /(d[oó]nde vives|eres de d[oó]nde|d[oó]nde eres)/i,
  /te puedo (conocer|escribir|hablar)/i,
  /c[oó]mo te (busco|encuentro|ubico)/i,
  /(a ver )?date la vuelta/i,
  /ponte (de espaldas|de perfil|al frente)/i,
  /(haces|har[ií]as) (ejercicio|deporte|gym|gimnasio)/i,
  /qu[eé] has hecho para estar (as[ií]|tan bien)/i,
];
